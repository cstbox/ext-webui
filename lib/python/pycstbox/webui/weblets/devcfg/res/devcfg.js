...
function setupCfgEditor(initData) {
    ...
    function information(message) {
        var dlg = $("#dlgInformation");
        $("#dlgInformation #message").text(message);
        dlg.dialog({
            autoOpen : true,
            resizable : false,
            width : 400,
    ...
    if (coord_types.length == 0) {
        //$("#device-editor-status").status('error', _i18n['no_meta']);
    ...
    function getDescriptorOfSelectedTreeNode(selectedItem) {
         ...
         * It can be : 
         * - a string equals to the id of the target element when
         * selection is made by id (eg: initially_select configuration option) 
         * - a single (or array of) jQuery objects when an item is clicked 
         *   In this case, the object can be : 
         *   + the <a> element when an item is selected with the mouse 
         *   + the parent <li> element when selection changed by code or 
         *     because it is closed while a child is selected
         */
...
//      console.log("selectedNodeIdAfterRefresh=" + selectedNodeIdAfterRefresh);
        ...
        if (selectedNodeIdAfterRefresh) {
             ...
             */
            setTimeout(function() {
                // we need to escape characters such as ':', '.' and '/' for jQuery to work
                var newDevNode = $("a#" + selectedNodeIdAfterRefresh.replace(/(:|\.|\/)/g, "\\$1"));
                nw_jstree.select_node(newDevNode);
//              console.log("selected=" + selectedNodeIdAfterRefresh);
...
//          console.log("select_node.jstree - nwNode=" + nwNode + " - type=" + nwNode.type + " - id=" + nwNode.id);
     ...
     */
    function appendEndPointFieldsetToDocument (
            epId, ep, propSet, dest, label, prefix, rules) {
    ...
    function editDevice(dev) {
        ...
        /*
         * Retreive the metadata for the device. They are used to build the
         * edition form
         */
        try {
        ...
        try {
            var propSet = devmeta.pdefs.root;
            ...
            fieldset.empty();
            appendFieldsetToDocument(dev, propSet, fieldset, "root", validatorRules);
        } 
        catch(err) {
        ...
        for (var section in sections) {
            ...
            if (epMeta) {
                ...
                if (epMeta.__count__) {
                    ...
                    var propSet = epMeta['*'];
                    for (var epId = 1; epId <= epMeta.__count__; epId++) {
                        var ep = deveps[epId];
        ...
        var deleteBtn = $("#btnDeleteDevice");
        deleteBtn.unbind('click');
        deleteBtn.click(function(event) {
            deleteDevice(dev.uid);
        });

        var chgIdBtn = $("#btnChangeDeviceId");
        chgIdBtn.unbind('click');
        chgIdBtn.click(function(event) {
            changeDeviceId(dev.uid);
        });
                    ...
                    $.ajax({
                        ...
                        success : function(data) {
                            ...
                            if (frm.data("visible"))
                                frm.fadeOut();
                            // refresh the tree
                            selectedNodeIdAfterRefresh = cid;
                            nw_jstree.refresh(-1);
                ...
                click: function() {
                    ...
                    if (newid != uid_parts[1]) {
                        $.ajax({
                            ...
                            success : function(data) {
                                // refresh the tree
                                selectedNodeIdAfterRefresh = cid + '/' + newid;
                                nw_jstree.refresh(-1);
                                
                                information($.format(_i18n['idchg_ok'], uid));
//                              $("#device-editor-status").status("info",
//                                      $.format(_i18n['idchg_ok'), uid));
        ...
        $.ajax({
            ...
            success : function(data) {
                if (data) {
                    ...
                    if (supportsDiscovery && data.discovery.ttl) {
                    ...
                    $("#subnet-editor-status").status("error", 
...
//      newDeviceFormValidator.resetForm();
                    ...
                    $.ajax({
                        ...
                        success : function(result) {
                            if (result.error) {
                                ...
                                $("#newDevAddr").val(sHex);// .attr("readonly", true);
    ...
    $("#btnCancelDiscovery").click(function() {
        ...
        discoveryProgressToggle(false);
        $("#subnet-editor-status").status("info", _i18n['operation_canceled']);
    });
